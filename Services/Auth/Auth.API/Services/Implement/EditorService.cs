...
public class EditorService : BaseService<Editor> , IEditorService
{
    private readonly IConfiguration _configuration;
    public EditorService(IUnitOfWork<DocAIAuthContext> unitOfWork, ILogger<Editor> logger, IMapper mapper, IHttpContextAccessor httpContextAccessor, IConfiguration configuration) : base(unitOfWork, logger, mapper, httpContextAccessor, configuration)
    {
    ...
    {
        var Viewers = await _unitOfWork.GetRepository<Editor>().GetPagingListAsync(
            selector: s => new Editor()
            {
                Id = s.Id,
                UserId = s.UserId,
                User = s.User,
    ...
    public async Task<EditorResponse> GetEditorInformationAsync()
    {
        ...
        var Viewer = await _unitOfWork.GetRepository<Editor>().SingleOrDefaultAsync(
    ...
    public async Task<EditorResponse> UpdateEditorAsync(UpdateEditorRequest request)
    {
        ...
        _unitOfWork.GetRepository<Editor>().UpdateAsync(Viewer);
